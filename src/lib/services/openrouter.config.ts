...
export function validateConfig(config: OpenRouterConfig): Required<OpenRouterConfig> {
  ...
  if (!config.apiKey || config.apiKey.trim().length === 0) {
    throw new Error("OpenRouter API key is required. " + "Please provide a valid API key in the configuration.");
  }
  ...
  if (config.baseUrl !== undefined) {
    try {
      ...
      throw new Error(`Invalid base URL: ${config.baseUrl}. ` + "Base URL must be a valid URL format.");
    }
  }

  // Validate timeout if provided
  if (config.timeout !== undefined && config.timeout <= 0) {
    throw new Error(`Invalid timeout: ${config.timeout}. ` + "Timeout must be a positive number in milliseconds.");
  }

  // Validate max retries if provided
  if (config.maxRetries !== undefined) {
    if (!Number.isInteger(config.maxRetries) || config.maxRetries < 0) {
      throw new Error(`Invalid maxRetries: ${config.maxRetries}. ` + "maxRetries must be a non-negative integer.");
    }
  }
