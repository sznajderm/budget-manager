...
export const GET: APIRoute = async () => {
  const results: Array<{ test: string; status: 'pass' | 'fail'; message: string; details?: unknown }> = []
  ...
  try {
    ...
    try {
      const response = await service.chat({
        messages: [
          { role: 'user', content: 'Say "OpenRouter integration verified!" and nothing else.' }
        ],
        temperature: 0.3,
        max_tokens: 50
      })
      ...
      results.push({
        test: 'Basic Chat Completion',
        status: 'fail',
        message: error instanceof OpenRouterError 
          ? `${error.code}: ${error.message}`
          : error instanceof Error 
            ? error.message 
            : 'Unknown error',
        details: error instanceof OpenRouterError ? {
          code: error.code,
