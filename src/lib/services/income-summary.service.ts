...
/**
 * Gets income summary for the authenticated user within a specified date range
 * @param supabase - Supabase client with user session
 * @param userId - User ID from authenticated session
 * @param summaryData - Validated summary command with date range
 * @returns Promise<SummaryDTO> - Aggregated income data with totals and counts
 */
export async function getIncomeSummary(
  ...
  try {
    // Query database for income transactions within date range
    ...
    if (error) {
      console.error("Database error retrieving income summary:", error);
      throw new Error("Failed to retrieve income summary from database");
    }
    ...
    console.error("Unexpected error in getIncomeSummary:", error);
    throw new Error("An unexpected error occurred while retrieving income summary");
  }
}
