...
describe("LoginForm", () => {
  ...
  describe("Form submission", () => {
    it("should submit form with valid credentials", async () => {
      const user = userEvent.setup();
      (global.fetch as any).mockResolvedValueOnce(new Response(JSON.stringify({ success: true }), { status: 200 }));
  ...
  describe("Server error handling", () => {
    ...
    it("should display default error message if response lacks error field", async () => {
      const user = userEvent.setup();
      (global.fetch as any).mockResolvedValueOnce(new Response(JSON.stringify({}), { status: 500 }));
