...
export class OpenRouterService {
  ...
  private parseResponse(rawResponse: unknown): ChatCompletionResponse {
    ...
    if (typeof response.created !== 'number') {
      throw new OpenRouterResponseError(
        'Invalid response: missing or invalid created field',
        rawResponse
      )
    }
    ...
    if (!response.usage || typeof response.usage !== 'object') {
      throw new OpenRouterResponseError(
        'Invalid response: missing or invalid usage field',
        rawResponse
      )
    }
    ...
    if (
      ...
      throw new OpenRouterResponseError(
        'Invalid response: invalid usage token counts',
        rawResponse
      )
    }
    ...
    if (
      typeof firstChoice.index !== 'number' ||
      !firstChoice.message ||
    ...
    if (
      ...
      throw new OpenRouterResponseError(
        'Invalid response: invalid message structure',
        rawResponse
      )
    }
  ...
  private validateMessages(messages: ChatMessage[]): void {
    ...
    messages.forEach((message, index) => {
      ...
      if (!message.content || message.content.trim().length === 0) {
        throw new OpenRouterValidationError(
          `Empty message content at index ${index}`,
    ...
    if (systemMessageCount > 1) {
      throw new OpenRouterValidationError(
        'Multiple system messages not allowed',
